...
def test_process_report() -> None:
    report: DataFrame = DataFrame(
        {
            "level": [1, 2, 3, solution.NULL_VALUE],
        },
    )
...
@pytest.mark.parametrize(
    "is_problem_dampener_enabled, expected",
    [
        (False, 2),
        (True, 4),
    ],
...
def test_get_answer(is_problem_dampener_enabled: bool, expected: int) -> None:
    assert solution.get_answer(TEST_INPUT, is_problem_dampener_enabled) == expected
